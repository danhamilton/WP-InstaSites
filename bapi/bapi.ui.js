...
context.inithelpers = {	
	...
	setuprateblockwidgets: function(options) {
		$.each($('.bapi-rateblock'), function (i, item) {
			...
			var templatename = ctl.attr('data-templatename');
			var selector = '#' + ctl.attr('id');
			BAPI.log("Creating rate block widget for " + selector, 3);
...
context.createRateBlockWidget = function (targetid, options) {
	...
	BAPI.datamanager.get(BAPI.entities.property, cur.ID, function(p) {
		...
		setCalendarsFromSession(BAPI.session.searchparams,'#rateblockcheckin','#rateblockcheckout');
			...
			onSet: function(event) {
				...
				if (theCheckOutElement!==null && theCheckOutElement.length>0) {
					...
					if ( event.select ) {
						...
						var theChkOutDate = moment(event.select).add('d',addDay).format('YYYY-MM-DD');
						theCheckOutElement.data('pickadate').set('select', theChkOutDate,{format:'yyyy-mm-dd' });
						//setting the min date for the checkout field 
						var theMinforCheckOut = moment(event.select).add('d',1);
						theCheckOutElement.data('pickadate').set('min', [theMinforCheckOut.year(),theMinforCheckOut.month(),theMinforCheckOut.date()]);
					}
...
function doSearch(targetid, ids, entity, options, alldata, callback) {
	...
	BAPI.get(ids, entity, options.searchoptions, function (data) {
		...
		if(showingMapView && ids.length > alldata.length && alldata.length < 99){
			var newAlldata = alldata.slice();
			var pagenumber = options.searchoptions.page + 1;
